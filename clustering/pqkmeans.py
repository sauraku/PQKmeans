...
import sklearn
import collections
from .pqkmeansCppToPython import PQKMEANS
...
class PQKMeans(sklearn.base.BaseEstimator, sklearn.base.ClusterMixin):
    def __init__(self, encoder, k, iteration=10, verbose=False):
        ...
        self._impl = PQKMEANS(self.encoder.codewords, k, iteration, verbose)
    ...
    def __setstate__(self, state):
        ...
        self._impl = PQKMEANS(state.encoder.codewords, state.k, state.iteration, state.verbose)
        self._impl.set_cluster_centers(state.cluster_centers)

    @property
    def labels_(self):
        return self._impl.GetAssignments()

    @property
    def cluster_centers_(self):
        return self._impl.GetClusterCenters()
